...
class TenantUserManager(BaseUserManager):
    ...
    def create_superuser(self, email, password=None, **extra_fields):
        """Superusers should log in with email, not phone number"""
        if not email:
...
class Tenant(AbstractUser):
    phone_no = models.CharField(max_length=11, unique=True, blank=False, primary_key=True)
    ...
    USERNAME_FIELD = "phone_no"  # Default for normal users
